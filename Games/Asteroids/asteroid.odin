...
createAsteroid :: proc(type: string, pos: rl.Vector2, min_vel, max_vel: i32) {
    ...
    if type == "big" do append_elems(&asteroids, Asteroids{{pos.x, pos.y, 122, 96}, vel, 0, type, true})
    if type == "med" do append_elems(&asteroids, Asteroids{{pos.x, pos.y, 42, 36}, vel, 0, type, true})
    if type == "sml" do append_elems(&asteroids, Asteroids{{pos.x, pos.y, ast_size.x, ast_size.y}, vel, 0, type, true})  
}
...
destroyAnimation :: proc(center: rl.Vector2, radius: f32, color: rl.Color, alive: bool, timer: int) {
    for i in 1..=10 {
        ...
        velocity := pos - center
        append_elems(&particles, Particles{pos, 1, color, velocity, alive, timer})
    }
}
...
drawParticles :: proc() {
    for &i, ind in particles {
        ...
        if i.alive == false do unordered_remove(&particles, ind)
    }
}